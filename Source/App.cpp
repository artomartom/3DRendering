...
public:
  ...
  void OnCursorMove(_In_ const ::Window::CursorArgs &args) noexcept
  {
    ...
    {
      m_mouse->CursorMoved(args.pos);
      m_camera.LookDirection(m_mouse->LookDirection());
      m_mouse->LastPos(args.pos);
    }
  };
  void OnCursorEvent(_In_ const ::Window::CursorArgs &args) noexcept
  {
    ...
    //case ::Window::CursorArgs::Event::LDown:
    //  // m_mouse->LastPos(args.pos);
    //  SetCapture(m_handle);
    //  break;
    //case ::Window::CursorArgs::Event::LUp:
    //  ReleaseCapture();
    //  break;
    }
  };
  ...
  void OnKeyStroke(_In_ const ::Window::KeyEventArgs &args) noexcept
  {
    switch (args.virtualKey)
    {
      ...
      CASE('C', {   SetCapture(m_handle); });
      CASE('R', {    ReleaseCapture(); });
    }
  };
  void OnCreate(_In_ const ::Window::CreationArgs &args) noexcept
  {
    ...
    if (H_OK(hr))
    {
      ...
      if (H_OK(hr = Renderer::Initialize()))
      {
        ...
        m_pImGui = std::make_unique<IMGUI::IMGUI>(m_pDeviceResource->GetDevice().Get(), m_pContext.Get(), m_handle, RECT{0, 0, 500, RTSize.cy});
        m_mouse = std::make_unique<Controller>(RECT{m_pImGui->GetWindowRect().right, m_pImGui->GetWindowRect().bottom, RTSize.cx, RTSize.cy});
#else
        m_mouse = std::make_unique<Controller>(RECT{0, 0, RTSize.cx, RTSize.cy});
#endif // IMGUI_DISABLE
...
private:
  std::unique_ptr<Controller> m_mouse{};
